...
def run(
    ...
    if influx is not None and len(influx) > 0:
        ...
        with InfluxDBClient(url=url, token=token, org=org, debug=True) as client:
            ...
            def _writer(rows):
                for row in rows:
                    point = f'{pair} bid={row[1]},ask={row[2]},bidSize={round(row[3], 4)},askSize={round(row[4], 4)} {int(row[0].timestamp() * 1000)}'
                    write_api.write(bucket=influx, record=point, write_precision=WritePrecision.MS)
